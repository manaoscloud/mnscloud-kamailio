...
usage() {
  cat >&2 <<'USAGE'
Usage: mnscloud-cdr-diagnostic-capture.sh --enabled yes --module softswitch --engine kamailio --resource-type softswitch_cdr --resource-uuid <id> --call-id <call-id> [--mode sip_capture|pcapng] [--interface any] [--duration 60] [--filter "port 5060"]
