...
O diagnóstico consulta `uac.reg_info l_uuid <UUID>` pelo UUID canônico da inscrição — o mesmo identificador usado
...
- salva contatos com `registrar/usrloc` em memória local usando AOR com domínio
...
`reg_db_url` e `reg_contact_addr`; o instalador usa `db_text` local via `text://` em
